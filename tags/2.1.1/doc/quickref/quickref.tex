...
\begin{center}
  \LARGE Getting started with NST\footnote{This document was last
    updated for NST version 2.1.0.  Written by John Maraist, Smart
...
\item\texttt{startup}, \texttt{setup}, \texttt{cleanup} and
  \texttt{finish} forms are run when the fixture set is applied to a
  group or test.  \texttt{Startup} is run before the fixtures' forms
  are evaluated; \texttt{setup} is run within the scope of the
  fixtures immediately after they are evaluated and bound;
  \texttt{cleanup} is the last form of the scope of the fixtures'
  bound names; \texttt{finish} is run after this scope is exited.
...
The \texttt{def-test-group} form \input{../common/group-syntax}
...
before (after) any of the individual tests.  The \texttt{startup},
\texttt{setup}, \texttt{cleanup} and \texttt{finish} forms are as for
fixtures.  Forms given with the \texttt{each-setup} (respectively
\texttt{each-cleanup}) option are run before (after) each individual
tests.

The \texttt{def-test} form defines a single named test:
\input{../common/check-syntax} The \texttt{startup}, \texttt{:setup},
\texttt{:cleanup}, \texttt{:finish} and \texttt{:fixture} options are
as above, but apply to only this test (and note that for multiple
