...
(def-documentation (compiler-macro def-output-framework)
  ...
  (:callspec (output-framework-name
              ...
              (:opt (:key-head :doc-title string))
              (:opt (:key-head :doc-author string))
              ...
              (:bag (:key-head :target-type name)
                    ...
                    (:key-head :grouping-label label-name)
  ...
  (:params (output-framework-name
           ...
           (doc-author "The author of the document.")
           ...
           (grouping-label "\\fbox{FILL IN}")
           (groups "\\fbox{FILL IN}")
           (default-group "\\fbox{FILL IN}")
           (with-output "\\fbox{FILL IN}"))
