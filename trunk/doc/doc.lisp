...
(defun build-nst-docs ()
  ...
  (let* ((doc-root-dir (asdf:system-relative-pathname (asdf:find-system :nst)
    ...
    (defdoc:write-output 'manual-primary-style 'the-manual doc-root-dir
                         "new-manual")
...
(defclass manual-style-mixin () ())

(defmethod get-latex-usepackage-specs ((style manual-style-mixin) item)
  (declare (ignore item))
  '(array (hyperref pdftex)))

(defmethod get-latex-primary-tocdepth ((style manual-style-mixin) item)
  (declare (ignore item))
  1)

(defmethod latex-parskip ((style manual-style-mixin) item)
  (declare (ignore item))
  "0.6em")

(defmethod defdoc-latex-contextualized::latex-parskip
    ((style manual-style-mixin) (context (eql :toc)) item)
  (declare (ignore item))
  "-0.6em")

(defmethod latex-parindent ((style manual-style-mixin) item)
  (declare (ignore item))
  "0pt")
