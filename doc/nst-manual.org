...
- Types =character= and =string=:
  ...
  - Argument =range=.  Allows the range of characters to be restricted
    ...
    |--------------+-------------------|
    # LaTeX: \begin{center}
    # LaTeX:   \begin{tabular}{>{\ttfamily:}c|l}
    # LaTeX:     \multicolumn{1}{c|}{Value} & \multicolumn{1}{c}{Meaning}
    # LaTeX:   \end{tabular}
    # LaTeX: \end{center}
...
- \label{run:primary}The =:run= command executes all tests in the
  named package, or in the named group, or runs the named test. It is
  not necessary to prefix the name with a package prefix.  The name
  does not need to be prefix-qualified, but if the name is ambiguous
  then =:run= will simply report the possible interpretations.
  #+begin_example
  :nst :run name
  #+end_example
- \label{run-package:primary} The =:run-package= command executes all
  tests associated with groups in the named packages, and reports the
  test results afterwards.
  #+begin_example
  :nst :run-package package ... package
  #+end_example
- \label{run-group:primary} The =:run-group= command executes all
  tests associated with the name groups, and reports the test results
  afterwards. The group name should be package-qualified.
  #+begin_example
  :nst :run-group group ... group
  #+end_example
- \label{run-test:primary} The =:run-test= command executes the given
  test.  Both the group and test name should be package-qualified.
  #+begin_example
...
- \label{apply:primary} The =:apply= command assesses whether a test
  criterion prints the uses to which a particular name has been
...
- \label{report:primary} The =:report= command summarizes successes,
  failures and errors in tests.  It reports either for the named
  artifact, or for all recently-run tests.
  #+begin_example
  :nst :report
  :nst :report package
  :nst :report group
  ...
  #+end_example
- \label{detail:primary} The =:report= command gives detailed
  information about individual test results.
  #+begin_example  :nst :detail
  :nst :detail package
  :nst :detail group
...
- \label{undef:primary}The =:undef= command retracts the definition of
  an NST group or test.
  #+begin_example
  :nst :undef group-name
  :nst :undef group-name test-name
  #+end_example

  Currently, NST does require that the symbols passed to =undef= be
  correctly package-qualified.

- \label{clear:primary} The =:clear= command empties NST's internal
...
- \label{set:primary} The =:set= command assigns or displays the
  values of NST runtime switches.
  #+begin_example
  :nst :set property
  :nst :set property value
  #+end_example
- \label{unset:primary} The =:unset= command clears the values of NST
  runtime switches.
  #+begin_example
  :nst :unset property
  #+end_example
...
- \label{verbose:primary}The =:verbosity= switch controls the level of
  NST's output.
  #+begin_example
  ...
  - =:vverbose=
  - =:trace=
  The =:report= and =:detail= commands operate by setting minimum
  levels of verbosity.
- \label{debug-on-error:primary}The =:debug-on-error= switch controls
  NST's behavior on errors.  When non-nil, NST will break into the
  debugger when it encounters an error.
  #+begin_example
  :nst :set :debug-on-error flag
  #+end_example
  The =:debug= command is a short-cut for setting this
  property.
- \label{debug-on-fail:primary} The =:debug-on-fail= switch controls
  NST's behavior when a test fails When non-nil, NST will break into
  the debugger when it encounters a failing test.
  #+begin_example
  :nst :set :debug-on-fail flag
  #+end_example
  This behavior is less useful than it may seem; by the time the
  results of the test are examined for failure, the stack from the
  actual form evaluation will usually have been released.  Still, this
  switch is useful for inspecting the environment in which a failing
  test was run.Note that both =:debug-on-error= and =:debug-on-fail=
  apply in the case of an error; if the latter is set but the former
  is not, then the debugger will be entered after an erring test
  completes.The =:debug= command is a short-cut for setting this
  property.
- \label{backtraces:primary} The =:backtraces= switch, when non-=nil=,
  directs NST to attempt to capture the Lisp backtrace of errors in
  tests.
  #+begin_example
...
- \label{*debug-on-error*:primary} User variable =*debug-on-error*=: if
  non-nil, will break into the Lisp REPL debugger upon encountering an
  unexpected error.  If nil, will record the error and continue with
  other tests.
- \label{*debug-on-fail*:primary} User variable =*debug-on-fail*=: if
  non-nil, will break into the Lisp REPL debugger upon encountering a
  test which fails.  If nil, will record the failure and continue with
  other tests.  This variable is useful inspecting the dynamic
  environment under which a test was evaluated.
- \label{*default-report-verbosity*:primary} User variable
  =*default-report-verbosity*= determines the default value for
  *nst-verbosity* when printing reports (2 by default).
- \label{*nst-output-stream*:primary} User =*nst-output-stream*=
  determines the output stream to which NST should print its
  =*standard-output*= by default).

Fixtures can be /opened/ into the interactive namespace for debugging:
#+begin_example
:nst :open FIXTURE-NAME FIXTURE-NAME ... FIXTURE-NAME
#+end_example
Example:
#+begin_example
